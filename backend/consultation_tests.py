...
r = client.get("/api/consultations/session/turn-config")
check("T1 turn-config without token -> 401", r.status_code == 401, f"got {r.status_code}")

ptok = patient_token()
r = client.get(f"/api/consultations/session/turn-config?token={ptok}")
...
# ---- 3. Authorization ------------------------------------------------------
r = client.get(f"/api/consultations/session/{cid}")
check("T6 get without token -> 401", r.status_code == 401, f"got {r.status_code}")

dtok = doctor_token()
r = client.get(f"/api/consultations/session/{cid}", params={"token": dtok})
check("T7 doctor can read consultation -> 200", r.status_code == 200, r.text[:100])

# ---- 4. End consultation ---------------------------------------------------
r = client.post(
    f"/api/consultations/session/{cid}/end",
...
with client.websocket_connect(f"/api/consultations/session/{cid2}/signaling?token={ptok}") as pws:
    ...
    with client.websocket_connect(f"/api/consultations/session/{cid2}/signaling?token={dtok}") as dws:
...
try:
    with client.websocket_connect(f"/api/consultations/session/{cid2}/signaling?token=bogus") as ws:
...
with client.websocket_connect(f"/api/consultations/session/{cid3}/signaling?token={ptok}") as pws2:
    pws2.receive_json()  # JOINED
    with client.websocket_connect(f"/api/consultations/session/{cid3}/signaling?token={dtok}") as dws2:
...
with client.websocket_connect(f"/api/consultations/session/{cid4}/signaling?token={ptok}") as pws3:
    pws3.receive_json()  # JOINED
    # Patient reconnects with a second socket - this replaces the first.
    with client.websocket_connect(f"/api/consultations/session/{cid4}/signaling?token={ptok}") as pws4:
        ...
        with client.websocket_connect(f"/api/consultations/session/{cid4}/signaling?token={dtok}") as dws4:
