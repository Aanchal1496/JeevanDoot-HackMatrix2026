...
async def main():
    ...
    cid = post('/api/consultations', {'appointment_id': 'APT-1', 'requester_role': 'patient'}, ptok)['consultation']['id']
    print(f'consultation {cid} created')

    print('STEP 3: patient WS')
    async with websockets.connect(f'{WS}/api/consultations/{cid}/signaling?token={ptok}', open_timeout=10) as p:
        ...
        async with websockets.connect(f'{WS}/api/consultations/{cid}/signaling?token={dtok}', open_timeout=10) as d:
